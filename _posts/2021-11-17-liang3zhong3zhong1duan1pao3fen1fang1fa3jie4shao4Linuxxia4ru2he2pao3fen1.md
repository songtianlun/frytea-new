...
UnixBench 是最初的 BYTE UNIX 基准套件，经过多年的更新和修订。测试结果是一个系统指标，而非 CPU、 RAM 或磁盘指标。因此测试结果不仅取决于硬件，还取决于操作系统、库，甚至编译器，评分结果能够反映器实际运行水平。
...
可以看到，单核跑分结果 1628.9 ，多核结果 5674.5 。炮分时可以用身边其他设备的分数作对比衡量跑分器性能。
...
同样用 Ubuntu 台式跑了一遍，可以在 [这里](https://browser.geekbench.com/v5/cpu/11025782) 查看跑分结果：
...
- [非常实用的Linux主跑分指南](https://post.smzdm.com/p/a27l8pd7/)
