...
`libvirt` 是目前使用最为广泛的对KVM虚拟进行管理的工具和 `API` 。 `Libvirtd` 是一个 `daemon` 进程，可以被本地的 `virsh` 调用，也可以被远程的 `virsh` 调用， `Libvirtd` 调用 `qemu-kvm` 操作虚拟。下面介绍 Centos7.6 下编译安装 Libvirt 7.5 的方法。
