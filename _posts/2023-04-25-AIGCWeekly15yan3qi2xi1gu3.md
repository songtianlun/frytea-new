...
jordan singer 的观点是：未来的界面将会是动态界面，可以根据提示直接生成 UI，这里的 UI 指的是代码，而不是设稿。比如一种用户界面类型。给定数据或 JSON，生成适合其最佳界面。一张照片 -- 照片界面 一个文章和链接的列表 -- 新闻界面 给定参数和限制条件，让 AI 根据提示、输入、用户和上下文变量决定界面。
